...
import { Box, Typography, TextField, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, IconButton, Menu, MenuItem, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Button, Pagination, InputAdornment } from "@mui/material";
				...
				<div className="content">
					<Box
								...
								<TableBody>
									{leaves.map((leave) => (
										<TableRow key={leave.id} sx={{ "&:last-child td, &:last-child th": { border: 0 } }}>
											...
											<TableCell>{leave.status === "active" ? <CheckCircle sx={{ color: "success.main" }} /> : <Cancel sx={{ color: "error.main" }} />}</TableCell>
											<TableCell>
												<IconButton aria-label="more" aria-controls="long-menu" aria-haspopup="true" onClick={(e) => handleMenuOpen(e, leave)}>
													<MoreVert />
												</IconButton>
											</TableCell>
										</TableRow>
						...
						<Menu id="simple-menu" anchorEl={anchorEl} keepMounted open={Boolean(anchorEl)} onClose={handleMenuClose}>
							...
							<MenuItem onClick={handleToggleStatus}>{selectedLeave && selectedLeave.status === "active" ? "Deactivate" : "Activate"}</MenuItem>
						</Menu>

						{/* Delete Confirmation Dialog */}
						<Dialog open={openDeleteDialog} onClose={handleDeleteCancel} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
							<DialogTitle id="alert-dialog-title">{"Confirm Delete"}</DialogTitle>
							<DialogContent>
								<DialogContentText id="alert-dialog-description">Are you sure you want to delete this leave record?</DialogContentText>
							</DialogContent>
							<DialogActions>
