...
<div class="page-content">
    ...
    <div class="">
        <div class="row justify-content-center">
            <div class="col-md-8">
                <form action="{{ route('booking.storeUserDetails') }}" method="POST" id="bookingForm"> @csrf
                    ...
                    <div class="card mb-4">
                        <div class="card-body p-4">
                            ...
                            <div class="mb-4">
                                ...
                                <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#branchModal"> Select Location </button>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="selection-area" id="branchSelect">
                        <h6>Select Branch</h6> @foreach($branches as $branch) <div class="btn-toggle" data-action="showFloors" data-branch-id="{{ $branch->id }}" data-branch-name="{{ $branch->name }}">
                            {{ $branch->name }}
                        </div> @endforeach
                    </div>
                    <div class="selection-area" id="floors" style="display: none;">
                        <h6>Select Floor</h6> @foreach($branches as $branch) @foreach($branch->floors as $floor) <div class="btn-toggle" data-action="showRooms" data-branch-id="{{ $branch->id }}" data-floor-id="{{ $floor->id }}" data-floor-name="{{ $floor->name }}">
                            {{ $floor->name }}
                        </div> @endforeach @endforeach
                    </div>
                    <div class="selection-area" id="rooms" style="display: none;">
                        <h6>Select Room</h6> @foreach($branches as $branch) @foreach($branch->floors as $floor) @foreach($floor->rooms as $room) <div class="btn-toggle" data-action="showTables" data-floor-id="{{ $floor->id }}" data-room-id="{{ $room->id }}" data-room-name="{{ $room->name }}">
                            {{ $room->name }}
                        </div> @endforeach @endforeach @endforeach
                    </div>
                    <div class="selection-area" id="tables" style="display: none;">
                        <h6>Select Table</h6> @foreach($branches as $branch) @foreach($branch->floors as $floor) @foreach($floor->rooms as $room) @foreach($room->tables as $table) <div class="btn-toggle" data-action="showChairs" data-room-id="{{ $room->id }}" data-table-id="{{ $table->id }}" data-table-name="{{ $table->name }}">
                            {{ $table->name }}
                        </div> @endforeach @endforeach @endforeach @endforeach
                    </div>
                    <div class="selection-area" id="chairs" style="display: none;">
                        <h6>Select Chair</h6> @foreach($branches as $branch) @foreach($branch->floors as $floor) @foreach($floor->rooms as $room) @foreach($room->tables as $table) @foreach($table->chairs as $chair) <div class="chair {{ $chair->status == 0 ? 'available' : 'reserved' }}" data-table-id="{{ $table->id }}" data-chair-id="{{ $chair->id }}" data-chair-name="{{ $chair->name }}" data-status="{{ $chair->status }}">
                            {{ $chair->name }}
                        </div> @endforeach @endforeach @endforeach @endforeach @endforeach
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                    <button type="button" class="btn btn-primary" id="confirmChairSelection" style="display: none;"> Confirm Selection </button>
                </div>
            </div>
        </div>
    </div>
